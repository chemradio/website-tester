...
import './ApptollBar.scss';

const AppToolbar = () => {
  const user = useSelector(state => state.users.user);
  const {setTheme} = useTheme();
  const [show] = useState(localStorage.getItem('app-theme'));

  const changeThemeNow = e => {
  ...
  return (
    <>
     <div className="allBlock">
           <Link to="/">
             <div style={{width: '50px', margin: '10px', backgroundColor: 'var(--toll-bar-background)'}}>
               <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 392.22 408.72" fill="var(--logo-color)">
         ...
         <Switcher seChange={changeThemeNow} value={show === "dark"}/>
